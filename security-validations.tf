...
locals {
  ...
  recovery_window_enforced = !var.security_control_overrides.disable_recovery_window_validation

  # Validation results
  kms_validation_passed             = !local.kms_encryption_required || var.kms_key_arn != null
  recovery_window_validation_passed = !local.recovery_window_enforced || var.recovery_window_in_days >= 7

  # Audit trail for overrides
  has_overrides = (
    var.security_control_overrides.disable_kms_requirement ||
    var.security_control_overrides.disable_recovery_window_validation
  )
...
check "security_controls_compliance" {
  ...
  assert {
    condition     = local.recovery_window_validation_passed
    error_message = "Security control violation: Recovery window must be at least 7 days but is set to ${var.recovery_window_in_days}. Set security_control_overrides.disable_recovery_window_validation=true with justification if this is intentional."
  }
